...
public class LogMessageText : MonoBehaviour
{
    ...
    /// <summary>
    /// Logメッセージイベント処理
    /// (log string, stack trace, log type)
    /// first  : Shows the contents of the log
    /// second : Shows the order in which they were called
    /// third  : Show log type.
    /// </summary>
    private void LogMessageOutput(string condition, string stackTrace, LogType type)
    {
        switch (type)
        {
            case LogType.Error:
                // ログメッセージとスタックトレースを表示
                condition = string.Format( "<color=red>{0}</color>", condition );
            ...
            case LogType.Warning:
                // ログメッセージのみ表示
                condition = string.Format( "<color=yellow>{0}</color>", condition );
